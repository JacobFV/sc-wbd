...
| **N-4a** | §6.1 (regional families are pretrained *on* their measurements) | Every haemodynamic source on disk is in **its own scanner space**; no source is parcellated into the model's region index. | Registration to the Schaefer parcellation needs a normalisation pipeline (FSL/ANTs/fMRIPrep) that this repository does not have and did not run. So the BOLD is *readable* (`scwbd.sources.loaders.bids_bold`, native voxel grid + native TR) and is **not yet trainable** against a region-indexed state. Recording this separately from N-4 because they fail differently: N-4 is "no data exists for this family", N-4a is "data exists, on disk, and the last mile is missing". Conflating them would let the second look like the first and never get fixed. | scheduled — blocks the haemodynamic likelihood, not the acquisition |
