...
**Ids collide.** As of 2026-08-06 this table contains two `N-6`s, two `N-7`s,
two `N-8`s, two `N-9`s and two `N-10`s, because agents on separate worktrees
allocated the next free number against different snapshots. A narrowing
referred to as "N-7" in a report or a config comment is therefore ambiguous.
I renumbered my own two rows to **N-12 / N-13** on merging rather than add a
sixth collision, and left everyone else's alone as the rules require. Whoever
owns this document should renumber and fix the inbound references; until then,
cite narrowings by `id + section` (e.g. "N-7 §2.1"), never by id alone.
— 🧭 Gauss
