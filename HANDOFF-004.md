> **This file is the run-4 handoff, and the mechanism it belongs to is superseded.**
...
NOW DO, in this order:
  ...
  2. `make release-004-evaluate`   -> reports/training/evaluation_run4.json
     READ `posterior_calibration` BEFORE writing any inference claim. ISSUE-012's
     calibration leg is decided here and nowhere else; the sweep could not
     measure it comparably.
  3. `make release-004-ablate`     -> evaluation_run4_ablation.json
     Already scores the MEASURED holdout, so step f's warning below is DISCHARGED
     in the target. ISSUE-016 gives it a candidate answer to check against.
  4. `make release-004-derived`    -> reports/scwbd-004_derived.json
     Paths derive from the checkpoint's run_name; it refuses a checkpoint without
     one. Guarded by tests/release/test_derived_report_follows_the_checkpoint.py.
  5. Model card via `plan_run4` — ISSUE-016 and ISSUE-012 up front, both OPEN.
  6. Site run-4 section; paper §11.10 (drafted PENDING, no numbers in it yet).
     Diff published bytes against freshly generated bytes before republishing.

  Run 5 is designed already, from run 4's four diagnostic arms, in
  reports/RUN5_DESIGN.md. Do not start it before run 4 has results.

=== STATUS 2026-08-10 15:00 ===
