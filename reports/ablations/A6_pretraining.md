# A6_pretraining — COULD_NOT_RUN

**Claim.** Does regional phenotype pretraining help beyond blank-slate training? (§11.4: region-specific pretraining versus end-to-end blank-slate training)
...
|---|---|---|---|
| arms[0] | yes | COULD_NOT_RUN | missing: region_specific_pretraining; §11.4 names it explicitly, so the comparison cannot be declared complete without it |
| arms[1] | yes | COULD_NOT_RUN | missing: end_to_end_blank_slate; §11.4 names it explicitly, so the comparison cannot be declared complete without it |
...
- arms[0]: could not run — missing: region_specific_pretraining; §11.4 names it explicitly, so the comparison cannot be declared complete without it
- arms[1]: could not run — missing: end_to_end_blank_slate; §11.4 names it explicitly, so the comparison cannot be declared complete without it
