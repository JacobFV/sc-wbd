...
- [The artifact and its generator are two objects](#the-artifact-and-its-generator-are-two-objects) — green tests on the generator are not evidence about the artifact
...
## The artifact and its generator are two objects

Three of today's structural findings came from the same move: comparing a
published artifact against the code that produces it, rather than checking
either alone. Each was invisible to every check that looked at only one side.

| what was compared | what it found |
|---|---|
| published card ↔ card directory the run used | release built its licence manifest from the directory `test_tiers.py` calls `LEGACY` |
| shipped checkpoint ↔ live source cards | `local.*` granted to a model whose module is `family_local` — 88.8% of parameters unreachable |
| **published card bytes ↔ freshly generated bytes** | the gradient-reach figure was being recomputed from *today's* cards |

The third is the sharpest, because it was a near-miss rather than a defect
already shipped.

### The near-miss

The published card said **88.8%** of run 2's parameters could not receive a
gradient. Regenerating the same card after the source-card patterns were
repaired produced **0.9%**.

Republishing — which had been done routinely all day — would have replaced the
artifact's headline finding with a number describing a run that never happened.

**Every other check passed.** Card live and fetching 200. Site live. Repo clean.
`tests/release` green. `make health` COMPLETE. Nothing was broken; the artifact
and the code had simply stopped agreeing, and no check in the project compared
them.

### Why one-sided checks cannot see it

A test exercises the *generator*: given these inputs, does it produce the right
output? It is correct, and it says nothing about the bytes already published,
because those were produced by a previous version of the generator from a
previous version of the inputs.

A liveness check exercises the *artifact*: is it there, does it fetch, is it
well-formed? Also correct, also silent, because a stale artifact is perfectly
well-formed.

> The two are separate objects with separate histories. Green tests on the
> generator are not evidence about the artifact, and a healthy artifact is not
> evidence about the generator. Only a diff between them is evidence about
> both.

### The rule that falls out

Anything an artifact says **about a finished run** must be computed from that
run's configuration, not from current configuration. The checkpoint records
`git_sha`; the cards at that commit are the ones that governed it. Where the sha
is unrecorded or unreadable, the honest output is a refusal to state the figure —
not a figure computed from whatever happens to be on disk.

Guarded by `tests/release/test_card_is_computed_from_the_run.py`, whose
discriminating assertion is that the figure stays above 50%: the live cards now
grant the renamed modules, so a low number can only mean it is being read from
them. Mutation-tested in the direction that matters — editing a live card must
**not** move the figure.

### The uncomfortable part

I found this on a hunch about staleness, not on suspicion of a bug, and only
after running every other check and reporting the artifact as verified. Had the
hunch not arrived, the next routine republish would have quietly overwritten the
day's principal finding with its own refutation.
