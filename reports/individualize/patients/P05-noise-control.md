# Individualization report -- patient `P05-noise-control`

## 1. Modalities present

| modality | source card | support | clock | calibration | declarations |
|---|---|---|---|---|---|
| `structural_mri` | declared:P05-noise-control:structural_mri:card | declared:P05-noise-control:structural_mri:support | declared:P05-noise-control:structural_mri:clock | declared:P05-noise-control:structural_mri:calibration | calibration=name,clock=name,source_card=name,support=name |
| `eeg` | declared:P05-noise-control:eeg:card | declared:P05-noise-control:eeg:support | declared:P05-noise-control:eeg:clock | declared:P05-noise-control:eeg:calibration | calibration=name,clock=name,source_card=name,support=name |

- absent: `['dmri', 'fmri', 'meg', 'behavior']` -- absent, not zero-imputed.
- reference-slice design: `eeg_only`, channels `['eeg']`
- sessions: `['s0']`
- record/model consistency: **REJECTED** -- whitened innovations mean square 1.981 over 96000 samples, tolerance 25% around 1
  - whitened innovations have mean square 1.9813 over 96000 samples (z = 215); OUTSIDE the declared 25% tolerance of 1: the record is not consistent with the forward model at any fitted parameter value, so nothing fitted through it is this patient's. The individualization is REJECTED and every group reverts to the population prior.
...
|---|---|---|---|---|
| `coupling` | POPULATION PRIOR -- NOT THIS PATIENT | population prior variance -- the record failed the model-consistency check, so no patient information was admitted | a21=100, a32=100, a13=100 | - |
| `conduction_delay` | POPULATION PRIOR -- NOT THIS PATIENT | population prior variance -- the record failed the model-consistency check, so no patient information was admitted | tau=0.0625 | - |
| `eeg_lead_field` | POPULATION PRIOR -- NOT THIS PATIENT | population prior variance -- the record failed the model-consistency check, so no patient information was admitted | gain_eeg=0.09, tilt_eeg=0.04 | - |
| `hemodynamic` | POPULATION PRIOR -- NOT THIS PATIENT | population prior variance -- the record failed the model-consistency check, so no patient information was admitted | beta_hrf=0.04, c_under=0.36, gain_bold=0.09 | - |
...
## Notes
- NEGATIVE CONTROL FIRED: whitened innovations have mean square 1.9813 over 96000 samples (z = 215); OUTSIDE the declared 25% tolerance of 1: the record is not consistent with the forward model at any fitted parameter value, so nothing fitted through it is this patient's. The individualization is REJECTED and every group reverts to the population prior.
...
# Identifiability profile -- patient `P05-noise-control`
...
```
Defer(no_action): query 'predicted_downstream_response' depends on parameter group(s) ['coupling', 'conduction_delay'], which were NOT individualized for patient 'P05-noise-control' from ['structural_mri', 'eeg']. NOT individualized: the negative control rejected this patient's record. whitened innovations have mean square 1.9813 over 96000 samples (z = 215); OUTSIDE the declared 25% tolerance of 1: the record is not consistent with the forward model at any fitted parameter value, so nothing fitted through it is this patient's. The individualization is REJECTED and every group reverts to the population prior. NOT individualized: the negative control rejected this patient's record. whitened innovations have mean square 1.9813 over 96000 samples (z = 215); OUTSIDE the declared 25% tolerance of 1: the record is not consistent with the forward model at any fitted parameter value, so nothing fitted through it is this patient's. The individualization is REJECTED and every group reverts to the population prior. Returning the model's value here would return the POPULATION value as though it were this patient's.
```
