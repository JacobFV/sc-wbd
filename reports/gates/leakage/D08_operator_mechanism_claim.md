# D08_operator_mechanism_claim — COULD_NOT_RUN

**Claim.** Operator / mechanism claim is controlled for. Primary metric: Timing, direction, dose/state dependence and unique intervention forecast.

**Falsified by (thesis).** The mandatory control (Equal-capacity generic operator, alternate mechanism and learned-residual controls; hold out discriminating perturbations) shows the result survives only without it.
...
- Delegated to gate A5 ablation; this Appendix D row and that gate are the same experiment and must not be double-counted as two pieces of evidence.
