...
\textbf{PENDING --- this subsection is filled from SC-WBD-004's completed run.
The finding below is measured; the run's held-out scores are not yet in and no
number from them appears here.}
...
is that it loses.} Across the founding stage \texttt{real\_bold\_nll} rises by two
orders of magnitude---\(1.99\) at step 1 to \(\approx\!200\) by step 1{,}000---while
the electrical likelihood \emph{improves} over the same span (\(1.74 \to 1.50\)),
the total objective stays flat near \(1.0\), and the haemodynamic variance channel
holds at \(5.3\)--\(5.9\). One term degrades monotonically while everything around
it improves, which is the signature of a source losing a competition rather than
of an optimiser failing.
