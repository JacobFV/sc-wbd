...
near-miss in our own first-gate artifact rather than a hypothetical: the
correction changed a difference of \(+9.22\) nats per observation between two
designs into a comparison whose pooling-free residual has the opposite sign,
while the correctly restricted comparison on the shared rows remained a
degradation (Section 11.5).
...
\(\alpha=0.05\), of which six were false positives. Two criteria failed.
Recovery interval coverage was \(0.16\), \(0.19\), \(0.53\) and \(0.50\) against
a nominal \(0.95\) over 32 simulated subjects. Held-out log loss did not beat the
baselines; and the first version of that comparison is the near-miss described
in Section 11.2, in which the joint and single-modality designs were compared
over different observation populations---845{,}280 rows against 844{,}800---so
that the extra 480 hemodynamic rows, whose own per-observation loss is
\(2.02\times10^{4}\) nats, produced an apparent degradation of \(9.22\) nats per
observation that pure pooling more than accounts for. Restricted to the common
844{,}800 rows the joint design remains worse than EEG alone, so the criterion
fails on the corrected comparison as well; but it fails for a measurable reason
rather than an arithmetic one, and the two must not be confused.
