...
def plan_run4(
    *,
    ...
    limitation = (
        ...
        "likelihood *improve*.\n>\n"
        "> `ds002336_real` appears in `contributed_sources` and that is accurate: "
        "its BOLD channel contributed a **gradient**. Whether that gradient "
        "carried information is a separate question, and this run's "
        "leave-one-source-out answers it: removing the corpus made **measured "
        "EEG prediction worse** (+0.0010), so the information went into the "
        "shared state and does not come back out through the BOLD head. Both "
        "things are true at once and neither licenses an fMRI claim.\n>\n"
