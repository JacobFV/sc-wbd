...
KNOWN_UNINFORMATIVE: tuple[UninformativeField, ...] = (
    ...
    UninformativeField(
        ...
        recurrence=(
            ...
            "being recomputed. "
            "SIXTH INSTANCE, and the first to change a SCIENTIFIC CLAIM rather than a "
            "statistic or a provenance field: 'naive resampling is rank 3/9' was relayed "
            "repeatedly WITHOUT ITS QUALIFIER. rank_likelihood = 3 holds under "
            "fisher_coarse_estimator (what the estimator can USE); under fisher_T4 the same "
            "design is rank 9 (what is AVAILABLE IN THE DATA). Stripping the qualifier "
            "INVERTS the finding -- it turns 'the estimator throws away information that is "
            "present' into 'the information is not there'. THE MECHANISM IS SPECIFIC: the "
            "failure is not only propagating unverified numbers, it is propagating them with "
            "the qualifiers removed, BECAUSE A QUALIFIER IS THE FIRST THING A SUMMARY DROPS. "
            "A number survives compression; the condition under which it holds does not. "
