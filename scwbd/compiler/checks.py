...
def check_r11(schema: BrainSchema, claim: ClaimManifest) -> Iterator[CompilerRefusal]:
    ...
    # ARCHITECTURE.md sec. 0: build-order item 6 is out of scope. A prospective
    # human stimulation protocol is refused unconditionally, override or not -
    # there is no IRB, no consent and no participants.
    for source, iv in interventions:
        if iv.is_prospective_human and iv.is_physical_stimulation:
            yield CompilerRefusal(
                ...
                offending_object=iv,
                detail=(
                    f"source {source.id!r} declares prospective human {iv.modality} "
                    "stimulation; SC-WBD-001-beta stops at build-order item 5 and no "
                    "agent may compile a human stimulation protocol"
                ),
                evidence={"source": source.id, "modality": iv.modality},
            )
    if claim.prospective_human:
        yield CompilerRefusal(
            ...
            detail=(
                "the claim manifest requests a prospective human protocol, which is "
    ...
    for source, iv in interventions:
        ...
        if iv.is_physical_stimulation and iv.ethics_review is None:
            yield CompilerRefusal(
                ...
                offending_object=iv,
                detail=(
                    f"source {source.id!r} optimizes a {iv.modality} exposure with no "
                    "applicable ethics and regulatory review on record"
                ),
                evidence={"source": source.id},
            )
